...
FEATURE_COLS = [
    "lag1_price", "lag2_price", "rolling_4w_mean", "deviation_from_rolling_mean",
    "rolling_4w_max", "price_vs_4w_max", "price_momentum", "consecutive_up_weeks",
    "week_of_year", "is_lean_season", "weeks_in_lean_season", "is_short_harvest_window",
    "weeks_to_long_harvest", "nairobi_lag1", "ug_lag1", "national_mean_lag1",
    "nairobi_ug_spread", "kamis_wholesale_lag1", "kamis_retail_lag1",
    "kamis_ug_supply_lag1", "kamis_national_supply_lag1", "is_kamis_augmented", "County",
]
CAT_COLS     = ["County"]
...
def main() -> None:
    ...
    train_enc, county_enc = encode_county(train)
    test_enc, _           = encode_county(test)
    ...
    for county in COUNTIES:
        ...
        log.info("%-15s  %6.3f  %8.4f  %8.4f  %8.4f", county, alpha, xgb_score,
                 carry_score, blend_score)
    ...
    log.info("Local score — XGB alone: %.4f | Blend: %.4f | Delta: %.4f",
             score_xgb, score_blend, score_xgb - score_blend)
    ...
    for county in COUNTIES:
        ...
        log.info("  %-15s  XGB errors: %s  |  Blend errors: %s",
                 county,
